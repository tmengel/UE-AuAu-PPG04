...
void SetBins(){
    ...
    for ( int i = 0; i < N_ET_BINS+1; ++i ) { ET_BINS[i] =  i*(MAX_ET)/N_ET_BINS; }
}
...
void EmbeddingPlots() 
{
    ...
    const std::string & input_file = "/sphenix/user/tmengel/UE-AuAu-PPG04/rootfiles/FEB7/feb7_embed.root";
    ...
    ConfigureOutputDirs("Embed");
    std::cout << "Embed plots: " << probe_plots << std::endl;

    SetBins();
    
    std::string probe_hist = ProcessEmbedTree(input_file, "embed");
...
std::string ProcessEmbedTree(const std::string & input_file, const std::string & prefix  )
{
    ...
    TH3F * h2_et_ntruth_cemc_cent = new TH3F("h2_et_ntruth_cemc_cent", "h2_et_ntruth_cemc_cent", N_X_CENT_BINS, X_CENT_BINS, N_ET_BINS, ET_BINS, N_CONECOMP_SUB1_BINS, CONECOMP_SUB1_BINS);
    TH3F * h2_et_ntruth_hcalin_cent = new TH3F("h2_et_ntruth_hcalin_cent", "h2_et_ntruth_hcalin_cent", N_X_CENT_BINS, X_CENT_BINS, N_ET_BINS, ET_BINS, N_CONECOMP_SUB1_BINS, CONECOMP_SUB1_BINS);
    TH3F * h2_et_ntruth_hcalout_cent = new TH3F("h2_et_ntruth_hcalout_cent", "h2_et_ntruth_hcalout_cent", N_X_CENT_BINS, X_CENT_BINS, N_ET_BINS, ET_BINS, N_CONECOMP_SUB1_BINS, CONECOMP_SUB1_BINS);
    ...
    for ( int i = 0; i < nentries; ++i ) {
        ...
        for ( unsigned itruth = 0; itruth < ntruth; itruth++) {
            ...
            // h2_et_ntruth_cent->Fill(xaxis_var, et_cemc, ncomp);
            ntruth_matched++;

...
const std::vector < float > calo_window_fit_bounds = {
    1, 4, 12, 30, 56, 90, 132, 169
}; 
...
std::string ProcessWindowTreeMBD(const std::string & input_file, const std::string & outfile = "MBD-window.root", const std::string & prefix = "rootfiles");
...
void WindowFitSys(const std::string input_file, const std::string & prefix);
void WindowFitsMBD(const std::string input_file, const std::string & prefix);
void DeltaFits(const std::string input_file,  const std::string & prefix);


void PPG04_Plots_Window() {
    ...
    WindowFitSys(window_file, "windowsys");
...
std::string ProcessProbeTree(const std::string & input_file, const std::string & outfile, const std::string & prefix )
{
    ...
    for ( auto h2 : h2s ) {
        ...
        h2->GetYaxis()->SetTitle("#delta E_{T,Uncorr.}^{Probe} [GeV]");
    }
...
std::string ProcessEmbedTree(const std::string & input_file, const std::string & outfile, const std::string & prefix  )
{
    ...
    for ( auto h2 : h2s ) {
        h2->GetXaxis()->SetTitle("Centrality [%]");
        h2->GetYaxis()->SetTitle("#delta E_{T,Uncorr.}^{Jet} [GeV]");
    }
    std::vector<std::string> labs = {"Area", "Multiplicity", "Iterative"};
...
std::string ProcessConeTree(const std::string & input_file, const std::string & outfile, const std::string & prefix  )
{
    ...
    for ( auto h2 : h2s ) {
        h2->GetXaxis()->SetTitle("Centrality [%]");
        h2->GetYaxis()->SetTitle("#delta E_{T,Uncorr.}^{Cone} [GeV]");
        
    }
    std::vector<TProfile*> p2s = {p2_area_res, p2_mult_res, p2_sub1_res};
    for ( auto p2 : p2s ) {
        p2->GetXaxis()->SetTitle("Centrality [%]");
        p2->GetYaxis()->SetTitle("#delta E_{T,Uncorr.}^{Cone} [GeV]");
    }
    std::vector<std::string> labs = {"Area", "Multiplicity", "Iterative"};
...
std::string ProcessPoisson(const std::string & input_file, const std::string & outfile, const std::string & prefix  )
{
    ...
    for ( auto g : g_poissions ) {
        ...
        g->GetYaxis()->SetTitle("#sigma(#delta E_{T,Uncorr.})");
    }
...
std::string ProcessWindowTreeMBD(const std::string & input_file, const std::string & outfile, const std::string & prefix  )
{
    ...
    int NEVENTS_CENT[N_SUM_ET_Q_BINS];
    for ( int i = 0; i < N_SUM_ET_Q_BINS; ++i ) { NEVENTS_CENT[i] = 0; }
    ...
    for ( unsigned int i = 0; i < k_window_array_size; ++i ) {
        ...
        h2_avg_et[i] = new TH2F(Form("h2_avg_et_%s", window_str.c_str()), Form("h2_avg_et_%s", window_str.c_str()),N_SUM_ET_Q_BINS, SUM_ET_Q_BINS, N_WINDOW_BINS, AVG_BINS);
        ...
        h2_sigma[i] = new TH2F(Form("h2_sigma_%s", window_str.c_str()), Form("h2_sigma_%s", window_str.c_str()),N_SUM_ET_Q_BINS, SUM_ET_Q_BINS, N_WINDOW_BINS, STD_BINS);
        ...
        h2_nwindows[i] = new TH2F(Form("h2_nwindows_%s", window_str.c_str()), Form("h2_nwindows_%s", window_str.c_str()),N_SUM_ET_Q_BINS, SUM_ET_Q_BINS, N_WINDOW_BINS, NUM_BINS);
        ...
        p2_avg_et[i] = new TProfile(Form("p2_avg_et_%s", window_str.c_str()), Form("p2_avg_et_%s", window_str.c_str()),N_SUM_ET_Q_BINS, SUM_ET_Q_BINS);
        ...
        p2_sigma[i] = new TProfile(Form("p2_sigma_%s", window_str.c_str()), Form("p2_sigma_%s", window_str.c_str()),N_SUM_ET_Q_BINS, SUM_ET_Q_BINS);
        ...
        p2_nwindows[i] = new TProfile(Form("p2_nwindows_%s", window_str.c_str()), Form("p2_nwindows_%s", window_str.c_str()),N_SUM_ET_Q_BINS, SUM_ET_Q_BINS);
        ...
        p2_avg_et2[i] = new TProfile(Form("p2_avg_et2_%s", window_str.c_str()), Form("p2_avg_et2_%s", window_str.c_str()),N_SUM_ET_Q_BINS, SUM_ET_Q_BINS);
    ...
    for ( int i = 0; i < nentries; ++i ) {
        t->GetEntry(i);
        float xaxis_var = mbd_q_N + mbd_q_S;
        if ( xaxis_var < SUM_ET_Q_BINS[0] || xaxis_var > SUM_ET_Q_BINS[N_SUM_ET_Q_BINS] ) { continue; }
        int THIS_BIN = -1;
        for ( int ibin = 0; ibin < N_SUM_ET_Q_BINS; ibin++ ) {
            if ( xaxis_var >= SUM_ET_Q_BINS[ibin] && xaxis_var <= SUM_ET_Q_BINS[ibin+1] ) {
                THIS_BIN = ibin;
                break;
            }
        }
        // if ( THIS_BIN < 0 || THIS_BIN >= N_SUM_ET_Q_BINS ) { continue; }
        if ( THIS_BIN < 0 || THIS_BIN > N_SUM_ET_Q_BINS ) { std::cout << "THIS_BIN: " << THIS_BIN << std::endl; continue; }
    ...
    TGraphErrors * g_sigma[N_SUM_ET_Q_BINS];
    TGraphErrors * g_avg_et[N_SUM_ET_Q_BINS];
    TGraphErrors * g_nwindows[N_SUM_ET_Q_BINS];
    TGraphErrors * g_avg_et2[N_SUM_ET_Q_BINS];
    for ( int icent = 0; icent < N_SUM_ET_Q_BINS; ++icent ) {
...
void WindowFitSys(const std::string input_file, const std::string & prefix)
{
    ...
    float fit_p0_tight[N_X_CENT_BINS];
    float fit_p0err_tight[N_X_CENT_BINS];
    float fit_chi2_tight[N_X_CENT_BINS];

    float fit_p0_avg[N_X_CENT_BINS];
    float fit_p0err_avg[N_X_CENT_BINS];

    const float fit_range_full[] = {0, 170};
    const float fit_range_tight[] = {10, 100};

    float fit_p0_ratio[N_X_CENT_BINS];
    float fit_p0_sys[N_X_CENT_BINS];
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        // and tight fit for the final value
    ...
    TGraphAsymmErrors  * g_sigma_fit_sys = new TGraphAsymmErrors (N_X_CENT_BINS);
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        g_sigma_fit_sys->SetPoint(i, X_CENT_BINS[i+1]-0.5*(X_CENT_BINS[i+1]-X_CENT_BINS[i]), fit_p0_full[i]);
        // asymmetric errors
        // if (fit_p0_sys[i] > 0) {
        //     g_sigma_fit_sys->SetPointError(i, 0, fit_p0err_full[i], 0 ); // positive error
        // } else {
            g_sigma_fit_sys->SetPointEXhigh(i, (X_CENT_BINS[i+1] - X_CENT_BINS[i]) / 2.0); // half bin width
            g_sigma_fit_sys->SetPointEXlow(i, (X_CENT_BINS[i+1] - X_CENT_BINS[i]) / 2.0); // half bin width
            g_sigma_fit_sys->SetPointEYlow(i, fit_p0_sys[i]); // no negative error
            g_sigma_fit_sys->SetPointEYhigh(i, fit_p0_sys[i]); // no negative error
        // }
    }
    ...
    // g_sigma_fit_sys->SetFillColorAlpha(kBlue-6, 0.8); // semi-transparent fill
    g_sigma_fit_sys->SetFillColor(kBlue-6); // solid fill
    ...
    g_sigma_fit_sys->SetFillStyle(3001); // 3001 is a filled band style
    ...
    // make errors a bar
    // g_sigma_fit_sys->SetFillColor(kBlack);

    g_sigma_fit_sys->Draw("AP2"); // draw as a filled band
    g_sigma_fit->Draw("PESAME"); // draw as a filled band

    // g_sigma_fit->Draw(""); // draw the points on top of the band
    tx = 0.19;
    ty_start = 0.25;
    for ( auto tag : tags ) {
    ...
    c->SaveAs(Form("%s/sigma_fits_sys.pdf", outdir.c_str()));
...
void WindowFits(const std::string input_file, const std::string & prefix)
{
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        std::string cent_title = Form("%d-%d%% Central", int(X_CENT_BINS[i]), int(X_CENT_BINS[i+1]));
        c = new TCanvas("c", "c", 800, 600);
        gPad->SetLeftMargin(0.15);
        gPad->SetRightMargin(0.15);
        gPad->SetBottomMargin(0.15);
        ...
        tex->DrawLatex(tx, ty, Form("k_{fit} = %0.2f", fit_p0_full[i]));
    ...
    std::vector<float> y_stat_errs{};


    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        g_sigma_fit->SetPoint(i, X_CENT_BINS[i+1]-0.5*(X_CENT_BINS[i+1]-X_CENT_BINS[i]), fit_p0_full[i]);
        x_centers.push_back(X_CENT_BINS[i+1]-0.5*(X_CENT_BINS[i+1]-X_CENT_BINS[i]));
        x_errs.push_back(0.5 * (X_CENT_BINS[i+1] - X_CENT_BINS[i])); // half width of the bin
        y_vals.push_back(fit_p0_full[i]);
        y_stat_errs.push_back(fit_p0err_full[i]);
        g_sigma_fit->SetPointError(i, 0, fit_p0err_full[i]);
    }
    g_sigma_fit->GetXaxis()->SetNdivisions(505);
    g_sigma_fit->GetYaxis()->SetNdivisions(505);
...
void WindowFitsMBD(const std::string input_file, const std::string & prefix)
{
    ...
    float fit_p0_full[N_SUM_ET_Q_BINS];
    float fit_p1_full[N_SUM_ET_Q_BINS];
    float fit_p0err_full[N_SUM_ET_Q_BINS];
    float fit_p1err_full[N_SUM_ET_Q_BINS];
    float fit_chi2_full[N_SUM_ET_Q_BINS];
    float fit_ndf_full[N_SUM_ET_Q_BINS];
    float fit_x0_full[N_SUM_ET_Q_BINS];
    float fit_xf_full[N_SUM_ET_Q_BINS];

    float fit_p0_res[N_SUM_ET_Q_BINS];
    float fit_p1_res[N_SUM_ET_Q_BINS];
    float fit_p0err_res[N_SUM_ET_Q_BINS];
    float fit_p1err_res[N_SUM_ET_Q_BINS];
    float fit_chi2_res[N_SUM_ET_Q_BINS];
    float fit_ndf_res[N_SUM_ET_Q_BINS];
    float fit_x0_res[N_SUM_ET_Q_BINS];
    float fit_xf_res[N_SUM_ET_Q_BINS];


    TF1 * fit_sigma;
    for ( int i = 0; i < N_SUM_ET_Q_BINS; ++i ) {
    ...
    for ( int i = 0; i < N_SUM_ET_Q_BINS; ++i ) {
        std::string cent_title = Form("%d-%d%%", int(SUM_ET_Q_BINS[i]), int(SUM_ET_Q_BINS[i+1]));
        c = new TCanvas("c", "c", 800, 600);
        gPad->SetLeftMargin(0.15);
        gPad->SetRightMargin(0.15);
        gPad->SetBottomMargin(0.15);
    ...
    gPad->SetBottomMargin(0.15);
    gPad->SetTopMargin(0.05);
    TGraphErrors * g_sigma_fit = new TGraphErrors(N_SUM_ET_Q_BINS);
    for ( int i = 0; i < N_SUM_ET_Q_BINS; ++i ) {
        g_sigma_fit->SetPoint(i, SUM_ET_Q_BINS[i+1]-0.5*(SUM_ET_Q_BINS[i+1]-SUM_ET_Q_BINS[i]), fit_p0_full[i]);
        g_sigma_fit->SetPointError(i, 0, fit_p0err_full[i]);
    }
    g_sigma_fit->GetXaxis()->SetNdivisions(505);
    g_sigma_fit->GetYaxis()->SetNdivisions(505);
    g_sigma_fit->GetXaxis()->SetTitle("#Sigma Q_{MBD}");
    g_sigma_fit->GetYaxis()->SetTitle("k_{fit}");
...
void DeltaPlots(const std::string input_file_basic, const std::string input_file_random, const std::string input_file_probe, const std::string & input_file_embed, const std::string & prefix)
{
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        for ( auto h1 : h1s ) {
            ...
            h1->GetXaxis()->SetTitle("#delta E_{T,Uncorr.} [GeV]");
        ...
        // // scale (x axis - mean)/mean
        // for (auto h1 : h1s_area) {
        //     float mean = h1->GetMean();
        //     float rms = h1->GetRMS();
        //     for ( int j = 0; j < h1->GetNbinsX(); ++j ) {
        //         float x = h1->GetBinCenter(j+1);
        //         float y = h1->GetBinContent(j+1);
        //         float yerr = h1->GetBinError(j+1);
        //         if ( mean != 0 ) {
        //             h1->SetBinContent(j+1, y/(x/mean));
        //             h1->SetBinError(j+1, yerr/(x/mean));
        //         }
        //     }
        // }
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        std::cout << "chi2/ndf = " << chi2overndf << std::endl;
        ...
        h1_area_basic[i]->GetYaxis()->SetRangeUser(1e-4, 1.9e1);
        ...
        leg->AddEntry(h1_a_embed, Form("Embed: #mu = %0.2f, #sigma = %0.2f", mean_a_embed, rms_a_embed), "pe");
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        h1_a_probe->Fit("fit_gamma", "QR", "", mean_a_basic-3*rms_a_basic, mean_a_basic+3*rms_a_basic);
        float a_a_basic = fit_gamma->GetParameter(0);
        float ab_a_basic = fit_gamma->GetParameter(1);
        float ap_a_basic = fit_gamma->GetParameter(2);
        float m = ap_a_basic/ab_a_basic;
        ...
        h1_mult_basic[i]->GetYaxis()->SetRangeUser(1e-4, 1.9e1);
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        h1_sub1_basic[i]->GetYaxis()->SetRangeUser(1e-4, 1.9e1);
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        h1_mult_random[i]->GetYaxis()->SetRangeUser(1e-4, 1.9e1);
        h1_mult_random[i]->SetMarkerSize(1.5);
        ...
        h1_sub1_random[i]->GetYaxis()->SetRangeUser(1e-4, 1.9e1);
        h1_sub1_random[i]->SetMarkerSize(1.5);
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        h1_mult_probe[i]->GetYaxis()->SetRangeUser(1e-4, 1.9e1);
        ...
        h1_sub1_probe[i]->GetYaxis()->SetRangeUser(1e-4, 1.9e1);
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        h1_area_embed[i]->GetYaxis()->SetRangeUser(1e-3, 1e0);
        ...
        h1_mult_embed[i]->GetYaxis()->SetRangeUser(1e-3, 1.9e1);
        ...
        h1_sub1_embed[i]->GetYaxis()->SetRangeUser(1e-3, 1.9e1);
...
void DeltaPlotsXcheck(const std::string input_file_basic, const std::string input_file_random,  const std::string & prefix)
{
    ...
    std::vector<TH2F*> h2s = {h2_area_res_basic, h2_area_res_random, h2_mult_res_basic, h2_mult_res_random, 
                            h2_sub1_res_basic, h2_sub1_res_random};
    int colors[] = {kRed, kAzure-2, kGreen+2};
    int markers[] = {kFullSquare, kOpenCircle, kFullCross, kFullTriangleUp};
    int markers_rc[] = {kFullSquare, kFullSquare, kFullSquare};
    int markers_random[] = {kOpenCircle, kOpenCircle, kOpenCircle};
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        std::vector<TH1F*> h1s = {h1_area_basic[i], h1_mult_basic[i], h1_sub1_basic[i], 
                                h1_area_random[i], h1_mult_random[i], h1_sub1_random[i]};
        ...
        std::vector<TH1F*> h1s_area = {h1_area_basic[i], h1_area_random[i] };
        std::vector<TH1F*> h1s_mult = {h1_mult_basic[i], h1_mult_random[i] };
        std::vector<TH1F*> h1s_sub1 = {h1_sub1_basic[i], h1_sub1_random[i]};
        for ( auto h1 : h1s_area ) {
            h1->SetLineColor(colors[0]);
            h1->SetMarkerColor(colors[0]);
        }
    ...
    for ( int i = 0; i < N_X_CENT_BINS; ++i ) {
        ...
        h1_a_basic_todraw->Divide(fit_gaus);
        h1_a_basic_todraw->SetMarkerStyle(markers[0]);
        h1_a_basic_todraw->SetMarkerSize(1.5);
        ...
        // take the ratio of the hist to the fit
        h1_a_random_todraw->Divide(fit_gaus2);
        h1_a_random_todraw->SetMarkerStyle(markers[1]);
        h1_a_random_todraw->SetMarkerSize(1.5);
        ...
        leg->AddEntry(h1_a_random_todraw, Form("Randomized #eta#phi: #mu = %0.2f, #sigma = %0.2f", mean_a_random, rms_a_random), "pe");

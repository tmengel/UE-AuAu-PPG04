...
void CaloWindowHistos() {
    ...
    // RunAll("/sphenix/user/tmengel/UE-AuAu-PPG04/rootfiles/FEB7/feb7_basic.root");
    ...
    RunAll("/sphenix/user/tmengel/UE-AuAu-PPG04/rootfiles/NEW/feb10_basic.root");
    gSystem->Exit(0);
...
void ProcessCaloWindowTree( const std::string & input_file, bool x_axis_cent )
{
    ...
    for ( unsigned int i = 0; i < k_window_array_size; ++i ) {
        ...
        float MAX_AVG  = (float)k_calo_window_dims_hcal_geom[i].first*k_calo_window_dims_hcal_geom[i].second*1.5;
...
void ProcessBackgroundTree(const std::string & input_file, bool x_axis_cent )
{
    ...
    TLegend * leg = new TLegend(0.25,0.7,0.5,0.92);
    leg->SetBorderSize(0);
    leg->SetFillStyle(0);
    leg->SetTextSize(0.04);
    leg->SetNColumns(2);

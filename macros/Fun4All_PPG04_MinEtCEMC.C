...
std::string ConvertMinEtToString(const double minEMCalEnergy )
{
    int EtMev = int(minEMCalEnergy * 1000.);
    std::ostringstream oss;
    if ( EtMev < 0 ){
        oss << "MinEMCalEt-OFF";
        return oss.str();
    }
    oss << "MinEMCalEt-" << EtMev << "MeV";
    return oss.str();
}

void Fun4All_PPG04_MinEtCEMC( 
    ...
    const double minEMCalEnergy = 0.05 
)
{

    std::string minEtTag = ConvertMinEtToString(minEMCalEnergy);
    ...
    PPG04Output::outfile = GetOutputFile( mode, prodTag, timeStamp, doRandomEtaPhi, outdir, dst_input_list0 , minEtTag );
    ...
    PPG04CaloSpy::outfile = GetOutputFile( mode, prodTag, timeStamp, doRandomEtaPhi, outdir, dst_input_list0 , minEtTag+"CALOSPY-");
    ...
    CaloManip::doMinEMCalEnergy = minEMCalEnergy > 0;
    // CaloManip::MinEMCalEnergy = 0.05; // nominal
    CaloManip::MinEMCalEnergy = minEMCalEnergy;

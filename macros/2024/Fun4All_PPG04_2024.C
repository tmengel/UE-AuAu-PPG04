...
)
{
    ...
    if (PPG04::isDATA) {
        ...
        CaloFittingOps::cemc_software_zs = 100;
        CaloFittingOps::ohcal_software_zs = 50;
        CaloFittingOps::ihcal_software_zs = 50;

        auto intrue2 = new Fun4AllRunNodeInputManager("DST_GEO");
        std::string geoLocation = CDBInterface::instance()->getUrl("calo_geo");
        intrue2->AddFile(geoLocation);
        se->registerInputManager(intrue2);

        Process_Calo_Fitting();
        // auto ca = new CaloFittingQA("CaloFittingQA");
        // se->registerSubsystem(ca);
        // CaloCalibOpts::is2024 = true;
        Process_Calo_Calib();
